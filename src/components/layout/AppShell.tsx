...
export function AppShell({
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col bg-background">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-border/60 bg-background/90 px-4 py-3 pt-[calc(0.75rem+env(safe-area-inset-top,0px))] backdrop-blur">
        <Logo size={40} />
      ...
      {!hideNav ? (
        <nav
          ...
          className="pointer-events-auto fixed bottom-0 left-1/2 z-30 w-full max-w-md -translate-x-1/2 border-t border-border/60 bg-card/95 backdrop-blur"
        >
          <ul className="flex items-stretch justify-between px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))]">
